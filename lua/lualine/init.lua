	...
	local function create_status_builder()
		...
		for _, section_name in ipairs(section_sequence) do
			if sections["lualine_" .. section_name] then
				...
				local section_highlight = highlight.format_highlight(is_focused, "lualine_" .. section_name)
				local section_data = utils_section.draw_section(sections["lualine_" .. section_name], section_highlight)
	...
	for i = 1, #status_builder do
		...
		if is_focused then
			...
			if current_section.name > "x" and config.options.section_separators[2] ~= "" then
				...
				if
					transitional_highlight
					and config.options.section_separators
					and config.options.section_separators[2]
				then
					table.insert(status, transitional_highlight .. config.options.section_separators[2])
				end
			end
			...
			if current_section.name < "c" and config.options.section_separators[1] ~= "" then
				...
				if
					transitional_highlight
					and config.options.section_separators
					and config.options.section_separators[1]
				then
					table.insert(status, transitional_highlight .. config.options.section_separators[1])
				end
			end
...
local function get_extension_sections()
	for _, extension in ipairs(config.extensions) do
		for _, filetype in ipairs(extension.filetypes) do
			local current_ft = vim.api.nvim_buf_get_option(vim.fn.winbufnr(vim.g.statusline_winid), "filetype")
