...
Diff.new = function(self, options, child)
	...
	new_instance.options.symbols = vim.tbl_extend("force", default_symbols, new_instance.options.symbols or {})
	if new_instance.options.colored == nil then
		new_instance.options.colored = true
	end
	-- apply colors
	if not new_instance.options.color_added then
		new_instance.options.color_added = utils.extract_highlight_colors("DiffAdd", "fg") or Diff.default_colors.added
	end
	if not new_instance.options.color_modified then
		new_instance.options.color_modified = utils.extract_highlight_colors("DiffChange", "fg")
			or Diff.default_colors.modified
	end
	if not new_instance.options.color_removed then
		new_instance.options.color_removed = utils.extract_highlight_colors("DiffDelete", "fg")
			or Diff.default_colors.removed
	end
	...
	if new_instance.options.colored then
		new_instance.highlights = {
			added = highlight.create_component_highlight_group(
				{ fg = new_instance.options.color_added },
				"diff_added",
				new_instance.options
			),
			modified = highlight.create_component_highlight_group(
				{ fg = new_instance.options.color_modified },
				"diff_modified",
				new_instance.options
			),
			removed = highlight.create_component_highlight_group(
				{ fg = new_instance.options.color_removed },
				"diff_removed",
				new_instance.options
			),
		}
...
Diff.update_status = function(self)
	...
	for _, name in ipairs({ "added", "modified", "removed" }) do
		if Diff.git_diff[name] and Diff.git_diff[name] > 0 then
			if self.options.colored then
				table.insert(result, colors[name] .. self.options.symbols[name] .. Diff.git_diff[name])
			else
...
function Diff.update_git_diff_getter()
	...
	Diff.get_git_diff = async:new({
		...
		on_stdout = function(_, data)
			if data then
				Diff.diff_data = Diff.diff_data .. data
			end
		end,
